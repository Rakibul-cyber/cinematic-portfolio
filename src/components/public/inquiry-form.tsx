...
export function InquiryForm({ services, turnstileSiteKey }: { services: PublicService[]; turnstileSiteKey: string | null }) {
  ...
  useEffect(() => {
    if (!state.success && state.message) window.turnstile?.reset();
  }, [state]);
  ...
  return <form action={action} className="grid gap-6" noValidate>
    ...
    {turnstileSiteKey ? <>
      ...
      <div className="cf-turnstile" data-action="inquiry" data-sitekey={turnstileSiteKey} data-theme="dark" />
      <p className="text-xs text-muted-foreground">Please complete the verification before sending.</p>
    </> : null}
